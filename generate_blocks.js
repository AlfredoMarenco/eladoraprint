...
export function IconListBlock({ data }: { data: any }) {
  ...
  'ContactFormBlock.tsx': `export function ContactFormBlock({ data }: { data: any }) {
    return (
                    ...
                    <form className="space-y-6" onSubmit={e => e.preventDefault()}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" className="w-full bg-[#FAF7F2] border-0 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#D4A853]" placeholder="Tu nombre" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Teléfono</label>
                                <input type="tel" className="w-full bg-[#FAF7F2] border-0 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#D4A853]" placeholder="Tu teléfono" />
                            </div>
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Correo Electrónico</label>
                            <input type="email" className="w-full bg-[#FAF7F2] border-0 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#D4A853]" placeholder="tu@correo.com" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Mensaje</label>
                            <textarea rows={4} className="w-full bg-[#FAF7F2] border-0 rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#D4A853]" placeholder="¿En qué te podemos ayudar?"></textarea>
                        </div>
                        <button className="w-full h-14 rounded-full bg-[#1A1A1A] text-white font-medium tracking-widest uppercase hover:bg-[#D4A853] transition-colors">
                            Enviar Mensaje
                        </button>
                    </form>
                </div>
                ...
                <div className="bg-[#FAF7F2] rounded-3xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6 border border-[#1A1A1A]/5 hover:border-[#D4A853]/30 transition-colors">
                    <div className="flex items-center gap-6 text-center sm:text-left">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-light text-[#1A1A1A]/60 mt-1">PDF Document</p>
                        </div>
                    </div>
                    <a href={data.file_url || '#'} className="inline-flex h-12 items-center justify-center rounded-full bg-white border border-[#1A1A1A] px-8 text-sm font-medium tracking-wider text-[#1A1A1A] uppercase transition-all hover:bg-[#1A1A1A] hover:text-white shrink-0">
                        Descargar
                    </a>
                </div>
            </div>
        ...
        <section className="bg-white py-12">
            <div className="container mx-auto px-4 max-w-7xl">
                {data.code ? (
                    ...
                    <div className="w-full p-12 bg-muted/20 border-2 border-dashed rounded-3xl flex items-center justify-center text-muted-foreground">
