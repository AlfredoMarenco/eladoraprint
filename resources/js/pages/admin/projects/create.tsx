...
import { index as adminProjectsIndex, store as adminProjectsStore } from '@/routes/admin/projects';
...
export default function Create({ categories }: { categories: any[] }) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(adminProjectsStore());
    };

    return (
        <>
            <Head title="Crear Proyecto - Admin" />
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                
                <div className="flex items-center gap-4">
                    <Button variant="outline" size="icon" asChild>
                        <Link href={adminProjectsIndex()}>
                            <ArrowLeft className="h-4 w-4" />
                        </Link>
                    </Button>
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-primary">Nuevo Proyecto</h1>
                        <p className="text-muted-foreground">Añade un nuevo trabajo a tu portafolio.</p>
                    </div>
                </div>
                    ...
                    <div className="md:col-span-2 flex flex-col gap-6">
                            ...
                            <CardContent className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <Input id="role" value={data.role} onChange={e => setData('role', e.target.value)} placeholder="Ej. Branding, Web Design" />
                                    </div>
                                    <div>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Sube imágenes adicionales. Puedes arrastrarlas y soltarlas.</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div {...getGalleryProps()} className="border-2 border-dashed rounded-md p-8 text-center cursor-pointer hover:bg-muted/50 transition-colors">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                        {galleryPreviews.map((preview, index) => (
                                            <div key={preview.id} className="relative aspect-square group rounded-md overflow-hidden border">
                                                ...
                                                <button type="button" onClick={() => removeGalleryImage(index)} className="absolute top-2 right-2 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <X className="h-4 w-4" />
                                                </button>
                                            </div>
                                        ))}
                    ...
                    <div className="flex flex-col gap-6">
                            ...
                            <CardContent className="flex flex-col gap-4">
                                ...
                                <div className="flex items-center space-x-2 mt-2">
                                    ...
                                    <Label htmlFor="is_published" className="font-normal cursor-pointer">
                                        Publicar en el portafolio inmediatamente
                                    </Label>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>La imagen principal que se verá en el listado.</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <div {...getCoverProps()} className="border-2 border-dashed rounded-md overflow-hidden bg-muted/20 relative aspect-video cursor-pointer hover:bg-muted/50 transition-colors flex items-center justify-center">
                                    <input {...getCoverInput()} />
                                    {coverPreview ? (
