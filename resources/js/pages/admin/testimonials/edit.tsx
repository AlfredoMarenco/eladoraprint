...
export default function Edit({ testimonial }: { testimonial: any }) {
    ...
    const [avatarPreview, setAvatarPreview] = useState<string | null>(testimonial.avatar_image ? `/storage/${testimonial.avatar_image}` : null);
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        post(admin.testimonials.update(testimonial.id), {
            onSuccess: () => toast.success('Testimonio actualizado exitosamente'),
        });
    };

    return (
        <>
            <Head title={`Editar Testimonio: ${testimonial.client_name}`} />
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-primary">Editar Testimonio</h1>
                        <p className="text-muted-foreground">{testimonial.client_name}</p>
                    </div>
                </div>
